...
if ( $1 ~ /MiB/ ) { 
      if (DEBUG==1) {printf("# MiB found. NR: %s NF: %s Value: %s\n",NR,NF,$1)}
      gsub("MiB","",$1)
      value_in_bytes = $1 * 1024 * 1024
      $1 = value_in_bytes
      if (DEBUG==1) {printf("# MiB found. NR: %s NF: %s Value: %s Value in bytes: %s\n",NR,NF,$1,value_in_bytes)}
      printf("# Value: %s\n",$1)
}
...
if ( $1 ~ /TiB/ ) { 
      if (DEBUG==1) {printf("# TiB found. NR: %s NF: %s Value: %s\n",NR,NF,$1)}
      gsub("TiB","",$1)
      value_in_bytes = $1 * 1024 * 1024 * 1024 * 1024
      $1 = value_in_bytes
      if (DEBUG==1) {printf("# TiB found. NR: %s NF: %s Value: %s Value in bytes: %s\n",NR,NF,$1,value_in_bytes)}
      printf("# Value: %s\n",$1)
}
